  ...
  let
    ...
    forAllSystems = f: nixpkgs.lib.genAttrs systems
      (system: f (import nixpkgs {
        inherit system;
        config = { cudaSupport = true; };  # turn on CUDA support globally
...
voxvibe-gnome-extension = final.stdenvNoCC.mkDerivation {
  ...
  '';
    ...
    let
      ...
      uuid  = lib.removeSuffix "\n" (builtins.readFile "${pkgs.voxvibe-gnome-extension}/nix-support/uuid");
    in {
      options.programs.voxvibe = {
        enable       = lib.mkEnableOption "VoxVibe dictation";
        package      = lib.mkPackageOption pkgs "voxvibe" { default = [ "voxvibe" ]; };
        model        = lib.mkOption { type = lib.types.str; default = "large-v3"; };        # best accuracy
        device       = lib.mkOption { type = lib.types.enum [ "auto" "cpu" "cuda" ]; default = "cuda"; };
        computeType  = lib.mkOption { type = lib.types.enum [ "auto" "int8" "int16" "float16" "float32" ]; default = "float16"; };
        hotkey       = lib.mkOption { type = lib.types.str; default = "<Super>h"; };        # Win+H analogue
        postProcess  = lib.mkOption { type = lib.types.bool; default = false; };             # off by default
      };

      config = lib.mkIf cfg.enable {
        home.packages = [
          cfg.package
          pkgs.voxvibe-gnome-extension
          pkgs.portaudio
        ];
    ...
    nixosModules.voxvibe = { config, lib, pkgs, ... }: {
      options.services.voxvibe.enable = lib.mkEnableOption "system-wide VoxVibe plumbing (via HM shared module)";
